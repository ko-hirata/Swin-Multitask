...
def inference(args, snapshot_path):
    ...
    with torch.no_grad():
        for i_batch, (image_batch, label_batch, _, _) in enumerate(testloader):
            ...
            predict_np = torch.argmax(outputs_cls).squeeze().to('cpu').detach().numpy().copy()
            save_dir = os.path.join(snapshot_path, f'inference/{label_np}{predict_np}')
            os.makedirs(save_dir, exist_ok=True)
            plt.imsave(save_dir + f'/{i_batch}.jpg', image_np, cmap="gray")
